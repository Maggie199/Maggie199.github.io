!function(){
	...
	var c1=[-220, 105], c2=[-30, 310], c3=[10, 360]; //Column positions of labels.
	...
	function drawPart(data, id, p){
		...
		mainbar.append("rect").attr("class","mainrect")
			.attr("x", 57).attr("y",function(d){ return d.middle-d.height/2; })
		...
		d3.select("#"+id).select(".part"+p).select(".subbars")
			...
			.attr("x", 57).attr("y",function(d){ return d.y})
	...
	function drawHeader(header, id){
		d3.select("#"+id).append("g").attr("class","header").append("text").text(header[2])
			.style("font-size","20").attr("x",158).attr("y",-45).style("text-anchor","middle")
		...
		[0,1].forEach(function(d){
			...
			h.append("text").text("Impact Factor").attr("x", (c2[d]-10))
				.attr("y", -5).style("fill","grey");
			
			h.append("line").attr("x1",c1[d]-10).attr("y1", -2)
				.attr("x2",c3[d]+40).attr("y2", -2).style("stroke","black")
				.style("stroke-width","1").style("shape-rendering","crispEdges");
		});
	}
	
	function edgePolygon(d){
		return [57, d.y1, bb+57, d.y2, bb+57, d.y2+d.h2, 57, d.y1+d.h1].join(" ");
	}	
